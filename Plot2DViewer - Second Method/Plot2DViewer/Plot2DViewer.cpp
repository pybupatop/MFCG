...
LRESULT _stdcall WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)		// оконная процедура принимает и обрабатывает все сообщения, отправленные окну
{
	...
	case WM_PAINT:
		{
			...
			scene.ElispsPlot(dc, Elipse);		
			//scene.ElispsPlot(dc, Spiral, 0.001, M_PI * 8);
			ReleaseDC(hWnd,dc);
	...
	case WM_MOUSEWHEEL:
		{
			int zDelta = GET_WHEEL_DELTA_WPARAM(wParam) / 4;
			if (GetKeyState(VK_UP)) 
			{
				scene.CompresionX(zDelta);
			}
			else if (GetKeyState(VK_DOWN))
			{
				scene.CompresionY(zDelta);
			} else 
			{
				scene.Zoom(zDelta);
			}
